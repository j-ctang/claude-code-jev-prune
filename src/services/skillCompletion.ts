...
export class SkillCompletionJudge {
  ...
  async score(goal: string, reply: string): Promise<number> {
        ...
        body: JSON.stringify({
          ...
          questions: {
            complete: {
              type: "noul",
              instructions:
                "Has the assistant completed current_goal in final_reply? Answer conservatively; partial work is incomplete.",
              criteria: {
                true: "The reply confirms the original task is fully completed.",
                false:
                  "Work remains, the reply is uncertain, or it asks for more information.",
              },
            },
          },
        }),
    ...
    const answer = (
      payload as { answers?: { complete?: { type?: unknown; noul?: unknown } } }
    )?.answers?.complete;
    if (
      answer?.type !== "noul" ||
