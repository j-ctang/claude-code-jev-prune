...
The observer runs independently of the existing pruner. It reads the request after existing pruning and records an occurrence only if a text span can be matched exactly, after harmless newline normalization, to a local `SKILL.md` body. It searches user skills and project skills only when all live launchers share one project. With multiple distinct projects, it skips project skill roots because request ownership is unknown. Identical content with the same skill name across roots counts as one logical skill; identical content with different names is ambiguous and skipped. Other unsupported content formats are skipped. It never records a skill body, prompt text, or file path in the log. It stores a skill identifier, session identifier, request occurrence, and estimated token count. The estimate uses the project's existing token estimator and is labeled approximate.
