...
const body = "Follow this detailed skill procedure when creating the report. Check every page, record any layout defect, and rerender before delivery.";
...
describe("SkillShadow", () => {
  ...
  test("identifies full skill body in a request and deduplicates repeat appearances", () => {
    ...
    expect(shadow.observe(request, "session-a")).toEqual([
      expect.objectContaining({ skill: "pdf", potentialTokens: Math.ceil(body.length / 4) }),
    ]);
    expect(shadow.observe(request, "session-a")).toEqual([]);
  });
  ...
  test("skips unknown and ambiguous skill bodies", () => {
    ...
    writeFileSync(join(other, "copy", "SKILL.md"), `---\nname: copy\n---\n${body}`);
    const shadow = new SkillShadow({ roots: [root, other], judge: async () => 1 });
    expect(shadow.observe({ messages: [{ role: "user", content: body }] }, "s")).toEqual([]);
    expect(shadow.observe({ messages: [{ role: "user", content: "unknown" }] }, "s")).toEqual([]);
    rmSync(other, { recursive: true, force: true });
  });

  test("records only high-confidence task completion", async () => {
    const low = new SkillShadow({ roots: [root], judge: async () => 0.8 });
    low.observe({ messages: [{ role: "user", content: body }] }, "s");
    expect(await low.complete("s", "Finished the report.")).toEqual([]);

    const high = new SkillShadow({ roots: [root], judge: async () => 0.99 });
    high.observe({ messages: [{ role: "user", content: body }] }, "s");
    expect(await high.complete("s", "Finished the report.")).toEqual([
      expect.objectContaining({ skill: "pdf", confidence: 0.99 }),
    ]);
    expect(await high.complete("s", "Finished the report.")).toEqual([]);
  });
