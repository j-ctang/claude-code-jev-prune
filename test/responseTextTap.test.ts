...
async function run(chunks: string[], contentType: string): Promise<{ output: string; finals: string[] }> {
  const finals: string[] = [];
  const output: Buffer[] = [];
  ...
  await pipeline(Readable.from(chunks), createResponseTextTap(contentType, (reply) => finals.push(reply)), sink);
...
test("streams bytes unchanged and captures a finished assistant text response", async () => {
  ...
  expect(await run(chunks, "text/event-stream")).toEqual({ output: chunks.join(""), finals: ["Done"] });
});

test("does not report an incomplete stream", async () => {
  const chunks = ['data: {"type":"content_block_delta","delta":{"type":"text_delta","text":"Working"}}\n\n'];
  expect((await run(chunks, "text/event-stream")).finals).toEqual([]);
});

test("captures a complete JSON message", async () => {
  const response = JSON.stringify({ stop_reason: "end_turn", content: [{ type: "text", text: "Done" }] });
